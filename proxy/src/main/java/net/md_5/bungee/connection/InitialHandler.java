...
//            if (!user.getName().equalsIgnoreCase(getName()) && user.getIp().equalsIgnoreCase(IPUtils.getAddress(this.ch).getHostAddress()))
//                playerAccountNumber++;
//
...
//        }
        ...
        //Auth start
        if (SettingsAuth.IMP.WHITELIST.ENABLE && !bungee.getAuth().getWhiteList().isPlayer(this.getName())) {
    ...
    private void finish() {
        ...
        } else if (this.bungee.getAuth().needCheck(this.getName(), this.getAddress().getAddress())) {
            this.sendLoginSuccess(!sendLoginSuccess); //Send a loginSuccess if sendLoginSuccess is false
            userCon.getCh().setEncoderProtocol(Protocol.GAME);
            userCon.getCh().setDecoderProtocol(Protocol.BotFilter);
            bungee.getAuth().connectToAuth(userCon, false);
        } else {
            this.bungee.getBotFilter().saveUser(userCon.getName().toLowerCase(), IPUtils.getAddress(userCon), false);
            AuthUser user = this.bungee.getAuth().getUser(userCon.getName());
            user.setIp(IPUtils.getAddress(userCon).getHostAddress());
            this.bungee.getAuth().saveUser(userCon.getName().toLowerCase(), user);
            userCon.setSession(true);
...
//        this.bungee.getAuth().sendEmailMsg(userCon);
    }
